#line 1 "Source\\gui_app\\LimitSpeedT.c"
...
#line 12 "Source\\gui_app\\LimitSpeedT.c"
...
#line 13 "Source\\gui_app\\LimitSpeedT.c"
...
void all_button(char able,int max)
 ...
 void TestCom(WM_MESSAGE * pMsg,ETestMode etest_mode_in)
 {
	 ...
	 {
	  lenth = LISTVIEW_GetNumRows(WM_GetDialogItem(pMsg->hWin,0x800+11));
	  if(lenth < 40)
	  {
		record.test_times++;
		...
		_ListBox[0]= buf;
		LISTVIEW_InsertRow(WM_GetDialogItem(pMsg->hWin,0x800+11),0,_ListBox);

		if(ttpars.dir !=2)
		{
		BUTTON_SetText(WM_GetDialogItem(pMsg->hWin,0x800+4),"正转中");
		ttpars.dir = 1;
		}else {
		BUTTON_SetText(WM_GetDialogItem(pMsg->hWin,0x800+4),"反转中");	
		}
		motor_dir(ttpars.dir);

		BUTTON_SetText(WM_GetDialogItem(pMsg->hWin,0x800+3),"运行中");
		start_test_init();
 ...
 void OnButtonTSaveClicked(WM_MESSAGE * pMsg,ESaveState save_state)
 {
	...
	char buf[10];
	hObj = WM_GetDialogItem(pMsg->hWin,0x800+11);
 ...
 void OnButtonDelectClicked(WM_MESSAGE * pMsg)
 {
	...
	if((get_data.e_work_state != START_TEST)&&(get_data.e_work_state != GET_V1))
	{
	row_number = LISTVIEW_GetNumRows(WM_GetDialogItem(pMsg->hWin,0x800+11));
	
	row = LISTVIEW_GetSel(WM_GetDialogItem(pMsg->hWin,0x800+11));
	
	LISTVIEW_DeleteRow(WM_GetDialogItem(pMsg->hWin,0x800+11),row);
	
	for(i=0;i<row_number-1;i++)
	 {
	    j = row_number-1-i;
	    sprintf(buf,"%d",j);
		LISTVIEW_SetItemText(WM_GetDialogItem(current_handle,0x800+11),0,i,buf);
	 }
 ...
 void OnButtonRTClicked(WM_MESSAGE * pMsg)
 {
    ...
    if(ttpars.dir == 1)
	{
	BUTTON_SetText(WM_GetDialogItem(pMsg->hWin,0x800+4),"反转");
	ttpars.dir = 2;
	}else {
	BUTTON_SetText(WM_GetDialogItem(pMsg->hWin,0x800+4),"正转");	
	ttpars.dir = 1;
	}
	motor_dir(ttpars.dir);
	}
 ...
 void OnButtonTStopClicked(WM_MESSAGE * pMsg)
 {
	...
	BUTTON_SetText(WM_GetDialogItem(pMsg->hWin,0x800+3),"停止");
	motor_dir(0);
...
{ FRAMEWIN_CreateIndirect,  "LIMIT_SPEED_H",     0,                       0,  0,  380,480,0,0},
    { BUTTON_CreateIndirect,    "跳变测试",              0x800+2,     1,  52, 100,40, 0,0},
    { BUTTON_CreateIndirect,    "停止",                0x800+3,     1,  190,100,40, 0,0},
    { BUTTON_CreateIndirect,    "正转/反转",             0x800+4,       1,  144,100,40, 0,0},
    ...
    { BUTTON_CreateIndirect,    "常规测试",              0x800+1,     1,  6,  100,40, 0,0}
};
...
void InitDialog_LimT(WM_MESSAGE * pMsg)
{
	...
	LISTVIEW_SetAutoScrollV(WM_GetDialogItem(hWin,0x800+11),1);

    LISTVIEW_SetFont(WM_GetDialogItem(hWin,0x800+11),&GUI_FontHZ_SongTi_16);
	
	LISTVIEW_SetRowHeight(WM_GetDialogItem(hWin,0x800+11),20);

	
	LISTVIEW_AddColumn(WM_GetDialogItem(hWin,0x800+11),20,"O",(3<<2)|(0<<0));
	LISTVIEW_AddColumn(WM_GetDialogItem(hWin,0x800+11),20,"M",(3<<2)|(0<<0));
	LISTVIEW_AddColumn(WM_GetDialogItem(hWin,0x800+11),50,"D",(3<<2)|(0<<0));
    LISTVIEW_AddColumn(WM_GetDialogItem(hWin,0x800+11),50,"V1",(3<<2)|(0<<0));
    LISTVIEW_AddColumn(WM_GetDialogItem(hWin,0x800+11),50,"V2",(3<<2)|(0<<0));
	LISTVIEW_AddColumn(WM_GetDialogItem(hWin,0x800+11),50,"Res",(3<<2)|(0<<0));
	LISTVIEW_AddColumn(WM_GetDialogItem(hWin,0x800+11),2,"---",(3<<2)|(0<<0));
...
{ 
   ...
   current_handle = GUI_CreateDialogBox(_aDialogCreate_LimT, (sizeof(_aDialogCreate_LimT) / sizeof(_aDialogCreate_LimT[0])), &_cbCallback_LimT, 0, 0, 0);
...
void draw_init_t()
{
	tdraw.buf_size = (800 - 400);
	tdraw.data_size = 0;
	tdraw.disp_kind = 1;
	tdraw.p0.x = 200+190+200+10;
	tdraw.p0.y = 21+(413);
	tdraw.p_rec_lu.x = 200+190;
	tdraw.p_rec_lu.y = 21;
	tdraw.p_rec_rd.x = 200+190+(800 - 400);
	tdraw.p_rec_rd.y = 21+(413);
