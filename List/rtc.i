...
#line 30 "Source\\BSP\\rtc.c"
#line 31 "Source\\BSP\\rtc.c"
#line 32 "Source\\BSP\\rtc.c"
...
unsigned char RTC_Get_Time(tim *timer)
{
	...
	RTC_WaitForLastTask();
	timecount=((RTC_TypeDef *) (((uint32_t)0x40000000) + 0x2800))->CNTH;
	timecount<<=16;
	timecount+=((RTC_TypeDef *) (((uint32_t)0x40000000) + 0x2800))->CNTL;
	...
	if(daycnt!=temp)
	{
		...
		while(temp>=28)
		{
			if(Is_Leap_Year(timer->year)&&temp1==1)
			{
				if(temp>=29)temp-=29;
				else break;
			}
	...
	timer->week=RTC_Get_Week(timer->year,timer->month,timer->date);
	return 0;
}
...
unsigned char RTC_Set(tim timer)
{
	...
	for(t=1970;t<timer.year;t++) 
	{
		if(Is_Leap_Year(t)!=0)seccount+=31622400;
		else seccount+=31536000; 
	}
	timer.month-=1;
	for(t=0;t<timer.month;t++) 
	{
		seccount+=(unsigned int)mon_table[t]*86400;
		if(Is_Leap_Year(timer.year)&&t==1)seccount+=86400;
	}
	seccount+=(unsigned int)(timer.date-1)*86400;
	seccount+=(unsigned int)timer.hour*3600;
	seccount+=(unsigned int)timer.min*60; 
	seccount+=timer.sec;

	
	
	((RCC_TypeDef *) ((((uint32_t)0x40000000) + 0x20000) + 0x1000))->APB1ENR|=1<<28;
	((RCC_TypeDef *) ((((uint32_t)0x40000000) + 0x20000) + 0x1000))->APB1ENR|=1<<27;
	((PWR_TypeDef *) (((uint32_t)0x40000000) + 0x7000))->CR|=1<<8; 
	
	((RTC_TypeDef *) (((uint32_t)0x40000000) + 0x2800))->CRL|=1<<4; 
	((RTC_TypeDef *) (((uint32_t)0x40000000) + 0x2800))->CNTL=seccount&0xffff;
	((RTC_TypeDef *) (((uint32_t)0x40000000) + 0x2800))->CNTH=seccount>>16;
	((RTC_TypeDef *) (((uint32_t)0x40000000) + 0x2800))->CRL&=~(1<<4);
	while(!(((RTC_TypeDef *) (((uint32_t)0x40000000) + 0x2800))->CRL&(1<<5)));
	return 0;
}
...
{
    if (BKP_ReadBackupRegister(((uint16_t)0x0004)) != 0xA5A1 )
	{
	    RTC_Configuration(); 
		RTC_WaitForLastTask();
		...
		BKP_WriteBackupRegister(((uint16_t)0x0004), 0xA5A1);
	}
	else
	{
		RTC_WaitForSynchro();        				
