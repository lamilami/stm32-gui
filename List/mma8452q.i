...
#line 3 "Source\\gui_app\\MMA8452Q.c"
#line 4 "Source\\gui_app\\MMA8452Q.c"
#line 1 "Source\\gui_app\\xyz_acc_para.h"
#line 2 "Source\\gui_app\\xyz_acc_para.h"
...
char put_head(short* in_buf, short* base_buf)
{
	memcpy(base_buf,base_buf+(63/2)+1,63/2);
	memcpy(base_buf+(63/2),in_buf,63+1);
}
...
{   
	...
	IIC_RegReadN(0x05, 2, z_buf);	
	IIC_RegReadN(0x01, 2, x_buf);
	IIC_RegReadN(0x03, 2, y_buf);
	...
	X_acc.acc_buf[X_acc.got_acc_num & (1024-1)] = x_temp;
	X_acc.got_acc_num++;
	}
...
{
    int xyz_status;
	xyz_status = IIC_RegRead(0x00);

    if(xyz_status & 0x04)
	{
		IIC_RegReadN(0x05, 2, buf);
		return true;
	}
...
int IIC_RegReadXYZ_or(uint32_t Out_xyz_msb_Reg,uint8_t *buf)
{
	...
	if(Out_xyz_msb_Reg == 0x05)
	{
		xyz_status &= 0x04;	
	}
	else if (Out_xyz_msb_Reg == 0x03)
	{
		xyz_status &= 0x02;
	}
	else if (Out_xyz_msb_Reg == 0x01)
	{
		xyz_status &= 0x01;
	}
	else
...
void Data_Sim(unsigned lenth)
 {
	...
	for(i=0;i<lenth;i++)
	{
		...
		Z_acc.acc_buf[Z_acc.got_acc_num & (1024-1)] =  temp;	
	...
	X_acc.acc_buf[X_acc.got_acc_num & (1024-1)] =  temp;	
	...
	Y_acc.acc_buf[Y_acc.got_acc_num & (1024-1)] =  temp;	
