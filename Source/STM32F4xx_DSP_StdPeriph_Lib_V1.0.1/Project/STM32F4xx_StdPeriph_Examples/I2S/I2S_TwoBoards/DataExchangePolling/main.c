/**
  ******************************************************************************
  * @file    I2S/I2S_TwoBoards/DataExchangePolling/main.c
...
__IO uint16_t RxMasterBuffer [RX_MASTER_BUFFERSIZE];
__IO uint16_t RxSlaveBuffer [RX_SLAVE_BUFFERSIZE];
__IO uint8_t uiBufferCounter = 0;
...
int main(void)
{
  ...
  Fill_Buffer((uint8_t*)RxMasterBuffer, (TX_MASTER_BUFFERSIZE*2));
  ...
  if (Buffercmp((uint8_t*)RxMasterBuffer, (uint8_t*)TxSlaveBuffer, (TX_SLAVE_BUFFERSIZE*2)) != FAILED) 
  {
  ...
  /* Clear the RxBuffer */
  Fill_Buffer((uint8_t*)RxSlaveBuffer, (TX_SLAVE_BUFFERSIZE*2));
  ...
  if (Buffercmp((uint8_t*)RxSlaveBuffer, (uint8_t*)TxMasterBuffer, (TX_SLAVE_BUFFERSIZE*2)) != FAILED) 
  {
    /* Turn ON LED3 */
    STM_EVAL_LEDOn(LED3);
  }
